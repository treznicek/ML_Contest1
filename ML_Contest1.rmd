...
training_matrix <- select(training_matrix, -most_recent_purchase)
...
average_weeks_between_purchases <- average_weeks_between_purchases %>%
  mutate(average_weeks = ifelse(is.na(average_weeks) | average_weeks == 0, 40, average_weeks))
...
training_matrix$average_weeks[is.na(training_matrix$average_weeks)] <- 40

summary(training_matrix$average_weeks)
```
...
```{r}
# Example list of features you want to train on
selected_features <- c("userID", "itemID", "target", "total_count", "unique_purchase_weeks", "weeks_since_last_purchase", "average_weeks")
