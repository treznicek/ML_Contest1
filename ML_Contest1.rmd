...
training_data <- orders_with_category[orders_with_category$date > as.Date("2020-11-10"),]
...
if(nrow(rows_with_zero_target) > 70000) {
  rows_with_zero_target_sampled <- sample_n(rows_with_zero_target, 70000)
} else {
...
train_control <- trainControl(
  ...
  search = "grid",         # Type of parameter search (grid or random)
  savePredictions = "final", # Save predictions for the optimal model
  allowParallel = TRUE    # Allow parallel processing if supported
)
...
rf_tuned_model <- train(
  x = predictors,
  y = response,
  method = "rf",           # Specify Random Forest method
  trControl = train_control,
  tuneGrid = param_grid,
  metric = "Accuracy"      # Optimization metric (e.g., Accuracy for classification)
)
